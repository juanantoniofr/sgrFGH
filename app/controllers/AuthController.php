...
class AuthController extends BaseController {
  ...
  */
  public function doLogin(){
    ...
    //Login database
    if (Config::get('options.ssologin') == false)
      return View::make('login/acceso');
  ...
  */
    ...
    if (Auth::attempt($data, Input::get('remember'))) // Como segundo parámetro pasámos el checkbox para sabes si queremos recordar la contraseña
      {
        ...
        return Redirect::intended(ACL::getHome());
      }
    // Si los datos no son los correctos volvemos al login y mostramos un error
    return Redirect::back()->with('error_message', 'Datos no válidos')->withInput();
  }
  ...
  public function doLogout(){
    ...
    if (Config::get('ssologin'))
      if (!Cas::isAuthenticated()) return View::make('wellcome');
      else Cas::logout();
    else return Redirect::to(route('loginsso'));;
