...
class ACL {
	...
	public static function getTipoActividadPorRol() {

		$aActividadesConAcceso = array();
		$aActividades = Config::get('options.tipoActividad');
	
		foreach ($aActividades as $aActividad) {
			$sRolesConAcceso = $aActividad['roles'];
			$aRolesConAcceso = str_split($sRolesConAcceso);
			if (in_array(Auth::user()->capacidad,$aRolesConAcceso) == true )
				$aActividadesConAcceso[] = $aActividad;
		}
	
		return $aActividadesConAcceso;
	}
	...
	//User delegación
 	...
 	public static function puedeSuperarLimiteHoras($recursoId){
 		
 		$idGrupoRecursos = Recurso::find($recursoId)->grupo_id;
		if (ACL::isAlumnDelegacion() && $idGrupoRecursos == '9') return true;
		return false;
 	}

	public static function puedeSuperarLimiteFechas($recursoId){
	...
	public static function puedeReservarMasDeUnRecursoAlaVez($recursoId){
	...
	public static function getHome(){
		...
		$home = Config::get('options.homeByrol')[Auth::user()->capacidad];
